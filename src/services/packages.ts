...
export type Package = {
  ...
  gallery: string[];
  included: string[];
  itinerary: string[]; // keys i18n — t(`packItinerary.${key}.title`)
};

type PackageRaw = Omit<Package, "gallery" | "included" | "popular" | "itinerary"> & {
  gallery: string;
  included: string;
  popular: number;
  itinerary: string;
...
function parse(raw: PackageRaw): Package {
  return {
    ...
    itinerary: JSON.parse(raw.itinerary),
  };
}

const SELECT = `
  SELECT slug, city, type, duration, nights, price, rating, reviews,
         popular, image, gallery, included, itinerary
  FROM packages
`;
