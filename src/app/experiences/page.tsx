...
const CATEGORIES: Array<Experience["category"] | "all"> = ["all", "Wellness", "Gastronomy", "Cultural", "Adventure"];
...
function ExperienciasContent() {
  ...
  const cat = (searchParams.get("cat") ?? "all") as Experience["category"] | "all";
  useEffect(() => { document.title = t("experiences.metaTitle"); }, [t]);

  const labelFor = (c: Experience["category"] | "all") =>
    c === "all" ? t("expCategory.all") : t(`expCategory.${c}` as const);

  const filtered = useMemo(() => {
    if (cat === "all") return experiences;
    ...
    <>
      ...
      <section className="container-page">
        <div className="flex flex-wrap gap-2">
          {CATEGORIES.map((c) => (
            <button key={c} onClick={() => router.push(`/experiences?cat=${c}`)}
...
export default function ExperienciasPage() {
  return <Suspense><ExperienciasContent /></Suspense>;
}